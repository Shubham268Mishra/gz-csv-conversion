...
# Define the path to the directory containing gzipped files
directory_path = r"folder_path"


# Function to process all .gz files in the specified directory
def process_gz_files_in_directory(directory_path):
    for file in os.listdir(directory_path):
        if file.endswith(".gz"):
            gzipped_file_path = os.path.join(directory_path, file)
            csv_output_file = os.path.join(
                directory_path, file[:-3] + ".csv"
            )  # Output CSV file path
            unzip_and_convert_to_csv(gzipped_file_path, csv_output_file)


# Call the function to process all .gz files in the specified directory
process_gz_files_in_directory(directory_path)
